...
def run(no_iter=5, no_voxels=1):
	print 'Begin data generation with ' + str(no_iter) + ' iterations and ' + str(no_voxels) + ' in every iteration'
	for i in range(0, no_iter):
		print('Begin iteration ' + str(i) + ' of ' + str(no_iter))
		dirname = "./data/gen/" + str(i) + '/'
		rand_rad = random.randrange(0, 10, 1)
		config = _get_config(voxels=no_voxels, cylinder_rad=rand_rad, dir_name=dirname)
...
def _get_config(walkers=100000, tmax=1000, voxels=1, p=0.0, scheme_path='hpc.scheme', cylinder_rad=1E-6, cylinder_sep=2.1E-6, dir_name=''):
	out_name = dir_name + 'cylinders.bfloat'
	obj = {
		'walkers': walkers,
		'tmax': tmax,
		'voxels': voxels,
